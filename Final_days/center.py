...
KP_PIX_TO_VEL        = 0.001   # m/s per pixel error (tune as needed)
...
def arm_and_takeoff(vehicle, target_alt):
    ...
    # wait until within 95% of target
    while True:
...
def main():
    ...
    print(f"Holding at ~{TAKEOFF_ALTITUDE} m, searching and centering on ArUco…")
    ...
    try:
        while True:
            ...
            if ids is not None and len(ids) > 0:
                ...
                # compute pixel error (marker relative to image center)
                dx = marker_px[0] - center_px[0]
                dy = marker_px[1] - center_px[1]

                # decide velocity in body frame
                vx = -KP_PIX_TO_VEL * dy  # positive dy => marker below => move forward (vx+)
                vy =  KP_PIX_TO_VEL * dx  # positive dx => marker right => move right (vy+)

                # cap velocities
                vx = max(min(vx, MAX_VEL), -MAX_VEL)
                vy = max(min(vy, MAX_VEL), -MAX_VEL)

                # if within threshold, we’re centered
