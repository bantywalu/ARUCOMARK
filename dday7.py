...
TARGET_MARKER_ID   = 0        # <<< only land on marker 0
...
def main():
    ...
    try:
        while True:
            ...
            # --- Print every detected marker ID ---
            if ids is not None:
                for mid in ids.flatten():
                    print(f"Detected Marker ID: {mid}")

            # --- Only keep marker 0 for landing check ---
            if ids is not None:
